...
$$ nb tubes = \frac{1.34*10^{-6}\cdot T \cdot \dot{n}}{7.854\cdot 10^{-3}} = 1.707*10{-4} \cdot T\cdot \dot{n}$$

Pour terminer cette partie, nous allons calculer le nombre de tuyaux pour ces données de base :

$$ \dot{n_{réactifs}} = \unit{872.22}{\mole\per\second} , T = \unit{1080}{K} , d_{tube} = \unit{10^{-1}}{\meter} et c = \unit{2}{\meter\per\second}$$

où $\dot{n_{réactifs}}$ est le débit molaire par jour de réactifs(déterminé avec l'outil de gestion pour une température citée ci-après et un débit massique de $NH_{3}$ de $\unit{1500}{\ton\per\dday}$, $T$ une température plausible choisie pour le dimensionnement du réacteur multi-tubulaire, $d_{tube}$ est le diamètre d'un tube et $c$ est la vitesse superficielle à l'entrée du réacteur.

Le formule du nombre de tuyaux devient:

$$ nb tuyaux = 1.707*10{-4} \cdot T\cdot \dot{n} = 160.83 $$
